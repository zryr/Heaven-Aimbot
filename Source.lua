--[[
    ...
    Version: 11.0 (Major Rebuild & Stability Release)
    Enhanced by: Gemini
]]
...
local function showWelcomeScreen(onFinished)
    ...
    local progressBar = Instance.new("Frame", welcomeFrame); progressBar.Size = UDim2.new(1, -40, 0, 4); progressBar.Position = UDim2.new(0.5, 0, 0.8, 0); progressBar.AnchorPoint = Vector2.new(0.5, 0.5); progressBar.BackgroundColor3 = Config.midBgColor; progressBar.BorderSizePixel = 0; local progressCorner = Instance.new("UICorner", progressBar); progressCorner.CornerRadius = UDim.new(1, 0)
    local progressFill = Instance.new("Frame", progressBar); progressFill.BackgroundColor3 = Config.accentColor; progressFill.BorderSizePixel = 0; progressFill.Size = UDim2.fromScale(0, 1); local fillCorner = Instance.new("UICorner", progressFill); fillCorner.CornerRadius = UDim.new(1, 0)
...
function MainGUI:Create()
    ...
    local listLayoutMain = Instance.new("UIListLayout", mainFrame); listLayoutMain.SortOrder = Enum.SortOrder.LayoutOrder; listLayoutMain.FillDirection = Enum.FillDirection.Vertical
    
    local dragFrame = Instance.new("Frame", mainFrame); dragFrame.Size = UDim2.new(1, 0, 0, 8); dragFrame.BackgroundColor3 = Config.accentColor; dragFrame.BackgroundTransparency = 1; dragFrame.ZIndex = 10; dragFrame.LayoutOrder = 1; dragFrame.Active = true; dragFrame.Draggable = true
    ...
    local header = Instance.new("Frame", mainFrame); header.Size=UDim2.new(1,0,0,40); header.BackgroundColor3=Config.midBgColor; header.Draggable=false; header.LayoutOrder = 2
    ...
    local scrollingFrame = Instance.new("ScrollingFrame", mainFrame); scrollingFrame.Size=UDim2.new(1,0,1,-48); scrollingFrame.BackgroundTransparency=1; scrollingFrame.BorderSizePixel=0; scrollingFrame.ScrollBarImageColor3=Config.accentColor; scrollingFrame.ScrollBarThickness=6; scrollingFrame.CanvasSize=UDim2.new(0,0,2,0); scrollingFrame.LayoutOrder = 3
...
function CoreLogic.applyHitboxMod(p)local c=p.Character;if not c then return end;local r=c:FindFirstChild("HumanoidRootPart");if r then if not originalHitboxSizes[p]then originalHitboxSizes[p]=r.Size end;pcall(function()r.Size=Vector3.new(Config.hitboxSize,Config.hitboxSize,Config.hitboxSize)end)end end
function CoreLogic.revertHitboxMod(p)if originalHitboxSizes[p]then local c=p.Character;if c and c:FindFirstChild("HumanoidRootPart")then pcall(function()c.HumanoidRootPart.Size=originalHitboxSizes[p]end)end;originalHitboxSizes[p]=nil end end
...
function CoreLogic.start()
    ...
    RunService.Heartbeat:Connect(function()if Config.hitboxModEnabled then for _,p in ipairs(Players:GetPlayers())do if p~=LocalPlayer then CoreLogic.applyHitboxMod(p)end end else for p,_ in pairs(originalHitboxSizes)do CoreLogic.revertHitboxMod(p)end end end)
end
...
showWelcomeScreen(function()
    ...
    CoreLogic.start()
    Players.PlayerRemoving:Connect(function(player)if originalHitboxSizes[player]then originalHitboxSizes[player]=nil end;if espHighlights[player]then espHighlights[player]=nil end end)
end)
